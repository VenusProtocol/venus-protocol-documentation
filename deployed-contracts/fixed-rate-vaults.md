...
Core contracts for the Fixed Rate Vaults system. Each institution's vault is deployed as an EIP-1167 clone by governance via `createVault(...)` on `InstitutionalVaultController`, so individual vault instances are not enumerated here — query the controller's `allVaults` array (or the `VaultCreated` event log) to enumerate deployed vaults.

## BNB Chain Mainnet

### Fixed Rate Vaults Contracts
