...
The bottom six are the setters `addPool` and `addMarket` drive with the registry as the caller; without them `addPool` reverts at execution. The pool's own policy setters (the allowlists, the per-market incentive) and `enterMarketForAccount` are separate grants on top of these. See [SpokeComptroller](spoke-comptroller.md#solidity-api).

## Integration notes

* **Reading the pool through the lens.** Spoke pools have their own lens, `SpokePoolLens`. [`PoolLens`](../lens/pool-lens.md) is unchanged and does not report the spoke-only state. `SpokePoolLens` keeps the `PoolLens` reads under the same names and struct shapes, so one lens address covers a spoke pool. The spoke-only reads are:
  * `getAllSpokePools(registry)` and `getSpokePoolByComptroller(registry, comptroller)` return `SpokePoolData`. It adds `deviationBoundedOracle`, `liquidationAllowlistEnabled` and the pool-wide `poolLiquidationIncentiveMantissa`.
  * `spokeVTokenMetadata(vToken)` returns `SpokeVTokenMetadata`. It adds `liquidationThresholdMantissa`, `effectiveLiquidationIncentiveMantissa`, `ownLiquidationIncentiveMantissa` (zero when the market uses the pool-wide value), `supplyAllowlistEnabled`, `forcedLiquidationEnabled` and `boundedPricingEnabled`. When `boundedPricingEnabled` is false, borrow and redeem in that market use the spot price.
  * `spokeSupplyPermissions(comptroller, vTokens, account)` tells whether an account may supply to each market. `spokeLiquidationPermission(comptroller, liquidator)` tells whether an account may seize collateral.

  Access-controlled permissions (for example who holds `enterMarketForAccount`) are not reported. Read them from the `AccessControlManager` directly.
* **Reading the pool directly.** `SpokeComptrollerViewInterface` collects the getters an integrator needs — the two allowlists, the effective liquidation incentive, the bounded oracle, `isForcedLiquidationEnabled`, plus `supplyCaps`, `actionPaused` and `markets` repeated so that consuming a spoke pool takes one import rather than three. `actionPaused` is declared there with a `uint8` action so a consumer does not have to import this repo's `Action` enum; the encoding is identical. `markets` is repeated because the shared declaration returns only the first two of its three fields and silently drops `liquidationThresholdMantissa`.
