...
{% hint style="info" %}
Venus Prime is being migrated to the PrimeV2 + PrimeLeaderboard architecture. The new contracts are currently live on BNB Chain testnet; rollout to other networks follows per-chain via governance.
{% endhint %}
...
A view of pending rewards is available so the [Venus UI](https://app.venus.io) can estimate the APR associated with a user's Prime boost. `getPendingRewards(user)` returns the pending reward per market (accruing first), and `getPendingRewardsStatic(user)` returns the same based on the last accrued state without triggering new accrual.

The steps to estimate APR are:

1. Fetch the income per block from PrimeLiquidityProvider
2. Calculate the user's yearly income by multiplying (1) by blocks per year
3. Calculate the user score and the total sum of scores for the market, giving the total reward allocation for the user over a year
4. Calculate the capped supply and borrow of the user for the market
5. Calculate the ratio of reward allocation based on capped supply and borrow of the user
6. Derive the borrow and supply APR from the capped borrow and supply ratios
...
Only the supply and borrow amounts below the cap generate Prime rewards. Amounts above the cap do not generate extra rewards. In the example, if the user supplies more USDT, they won't generate more rewards (because the supply amount considered is capped at 15 USDT). The supply APR would therefore decrease if they supply more USDT.
