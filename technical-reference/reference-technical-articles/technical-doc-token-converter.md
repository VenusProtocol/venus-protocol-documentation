...
The **Venus protocol** generates income in various underlying tokens, from interest reserves and liquidation fees, that subsequently conveyed to the [ProtocolShareReserve](https://bscscan.com/address/0xCa01D5A9A248a830E9D93231e791B1afFed7c446) contract. The `ProtocolShareReserve` contract disburses these earnings to number of destinations:
...
`RiskFundConverter` and `SingleTokenConverter` convert the incoming earnings into specific tokens, with `RiskFundConverter` converting them into RiskFund's convertible base asset and `SingleTokenConverters` converting them into their `baseAsset`. Each time a conversion gets complete, the resulting tokens are sent to their destination addresses, respectively.
...
- `PrimeLiquidityProvider` accumulates [the Prime funds](prime.md), distributing them according to the speeds configured via VIP.

# `AbstractTokenConverter`

`AbstractTokenConverter` contract provides every feature to configure and convert the desired assets. The `RiskFundConverter` and the `SingleTokenConverters` extend this abstract contract (that could also be extended by any other contract in the future because it provides their features in an agnostic way).
...
- `conversionAccess` (see more about private conversions below):
    - `NONE`: Conversion is disabled for the pair
    - `ALL`: Conversion is enabled for private conversion and users
    - `ONLY_FOR_CONVERTERS`: Conversion is enabled only for private conversion
...
The incentive is applied on the base out amount calculated using the conversion rate, calculated with the oracle prices. Incentives will be defined using normal VIP’s, following the Governance mechanism. Each pair of tokens can have a different incentive value. Bigger incentives can be set for converting tokens with lower liquidity, for example.

## Interaction with the token converter contracts

### Get amount out

View function `getAmountOut(uint256 amountInMantissa, address tokenAddressIn, address tokenAddressOut) returns (uint256 amountConvertedMantissa, uint256 amountOutMantissa)`, to request the amount of `tokenAddressOut` tokens that a sender would receive providing `amountInMantissa` tokens of `tokenAddressIn`, where:
...
- Returned values:
    - `amountConvertedMantissa`: the amount of `tokenAddressIn` that would be really transferred from the sender to the contract. If there is enough liquidity of `tokenAddressOut` in the contract (use the virtual function `balanceOf(address token)`), this amount is equal to `amountInMantissa`. Otherwise, it will be less than that amount
...
- Current status:
    - There is a configuration entry for the pair `tokenAddressIn / tokenAddressOut`: XVS/USDC. That means, the contract accepts conversions where the sender sends XVS to the contract, and the contract sends USDC to the sender. This will be a potential setup in the `XVSVaultConverter` contract.
...
- Invocation 1: `getAmountOut(15 * 10^18, address(XVS), address(USDC))`
    ...
    - Response:
        ...
        - `amountOutMantissa`: (15 * (5/1) * 10^18) * 1,01 = 75,75 * 10^18. That is **75,75 USDC**. Where:
...
- Invocation 2: `getAmountOut(25 * 10^18, address(XVS), address(USDC))`
    - It means: how many USDC would the sender receive if they provided **25 XVS** to the contract
    - In this case, there isn’t enough liquidity to use 100% of the input amount (25 XVS would be 126,25 USDC), so 100% of the liquidity will be the `amountOutMantissa`, and this function (`getAmountOut`) calculates the input amount needed to “cover” that amount (taking into account the discount)
    - Response:
        - `amountConvertedMantissa`: ((100 / 1,01) / 5) * 10^18 = **19,80 XVS**, where:
...
View function `getAmountIn(uint256 amountOutMantissa, address tokenAddressIn, address tokenAddressOut) returns (uint256 amountInMantissa, uint256 amountConvertedMantissa)`, to request the amount of `tokenAddressIn` tokens that a sender should send to the contract, to receive `amountOutMantissa` tokens of `tokenAddressOut`, where:
...
- Returned values:
    ...
    - `amountConvertedMantissa`: the amount of `tokenAddressOut` tokens that would be really transferred from the contract to the sender if the user sends `amountInMantissa` tokens of `tokenAddressIn` to the contract. If there is enough liquidity of `tokenAddressOut` in the contract, this amount is equal to `amountOutMantissa`. Otherwise, it will be less than the amount

Examples:

- Invocation 1: `getAmountIn(40 * 10^18, address(XVS), address(USDC))`
    - The configuration enables for `tokenAddressIn / tokenAddressOut`: XVS/USDC
    - Current liquidity: 100 USDC
    - Current conversion rate, given by the oracle: 1 XVS = 5 USDC
    - Configured discount: 0%
    - Response:
...
Function `convertExactTokens(uint256 amountInMantissa, uint256 amountOutMinMantissa, address tokenAddressIn, address tokenAddressOut, address to)`, where:
...
- `amountInMantissa`: the number of tokens (`tokensAddressIn`) the sender wants to convert
    ...
    - Examples:
        - 1 [TRX](https://bscscan.com/address/0xce7de646e7208a4ef112cb6ed5038fa6cc6b12e3) (6 decimals in BSC), will be 1000000
        - 1 [USDC](https://bscscan.com/address/0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d) (18 decimals in BSC), will be 1000000000000000000
- `amountOutMinMantissa`: the minimum amount of `tokenAddressOut` tokens the sender accepts to receive in the conversion. It’s defined in the mantissa of the `tokenAddressOut`.
    - If the amount that would be finally transferred to `to` is less than `amountOutMinMantissa`, the transaction is reverted
...
- It must satisfied the constraint of sending more than `amountOutMinMantissa` anyway
...
Function `convertForExactTokens(uint256 amountInMaxMantissa, uint256 amountOutMantissa, address tokenAddressIn, address tokenAddressOut, address to)`, where:
...
If the received amount or the transferred out amounts are less than the required amounts, the transaction is reverted, because we don’t support tokens with fees on transferring in this function.

There must be a `ConversionConfig` entry for the pair `tokenAddressIn / tokenAddressOut`. Otherwise, the transaction will be reverted.

This function could use internally the `getAmountIn` function. If the `amountConvertedMantissa` amount is less than the param `amountOutMantissa`, the transaction is reverted. This constraint is similar to this one: if the liquidity of `tokenAddressOut` is less than `amountOutMantissa`, then the transaction is reverted.

## Private Conversion

Private conversion aims to maximize the conversion of tokens received from the `ProtocolShareReserve` contract (to any converter) among existing converters in the same `ConverterNetwork` contract. This is done to preserve incentives and reduce user dependence on conversion. Converters will autonomously carry out Private Conversion whenever funds are received from `ProtocolShareReserve`, and no incentives will be provided during this process.
...
This contract contains the list of all the converters and will provide valid converters which can perform conversions. Converters will interact with this contract to get the list of other converters open for Private conversion.